(() => {
    ...
    (function initTyping() {
        ...
        if (isMobile) {
            ...
            animatedEl.classList.add("static-text");
            return;
        }
    ...
    function renderSummary() {
        ...
        if (showing === total) {
            summaryEl.textContent = `${showing} wallpaper${
                showing !== 1 ? "s" : ""
            }`;
        } else {
            summaryEl.textContent = `Showing ${showing} of ${total} wallpaper${
                total !== 1 ? "s" : ""
            }`;
        }
    }
    ...
    function renderGrid(items) {
        ...
        if (!items.length) {
          ...
          <p class="mt-3 mb-2">No wallpapers found.</p>
    ...
    function openPreviewAtIndex(idx) {
        if (!Array.isArray(filtered) || idx < 0 || idx >= filtered.length)
            return;
        currentIndex = idx;
    ...
    function applySearchSort() {
        ...
        filtered = data.filter((it) => {
            ...
            if (
                new Date(it.modified).toLocaleString().toLowerCase().includes(q)
            )
                return true;
            return false;
        });
