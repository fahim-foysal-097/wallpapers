(() => {
    ...
    function showToast(message, type = "success") {
        ...
        if (headerIcon) {
            headerIcon.className = `bi ${icon} me-2`;
        }
    ...
    function copyImageUrl() {
        ...
        navigator.clipboard
            ...
            .then(() => {
                ...
                if (modalCopyUrl) {
                    modalCopyUrl.classList.add("copied");
                    setTimeout(() => {
                        modalCopyUrl.classList.remove("copied");
                    }, 2000);
                }
            })
            .catch((err) => {
    ...
    function renderList(list) {
        ...
        <i class="bi bi-phone fs-1" aria-hidden="true"></i>
        <p class="mt-3 mb-2">No mobile wallpapers found.</p>
      ...
      `;
        ...
        if (showing === total) {
            summaryEl.textContent = `${showing} mobile wallpaper${
                showing !== 1 ? "s" : ""
            }`;
        } else {
            summaryEl.textContent = `Showing ${showing} of ${total} mobile wallpaper${
                total !== 1 ? "s" : ""
            }`;
        }
    }
    ...
    function openModalAtIndex(i) {
        ...
        if (modalMeta)
            modalMeta.textContent = `${Math.round(
                entry.size / 1024
            )} KB • ${new Date(entry.modified).toLocaleString()}`;
        if (modalDownload) {
        ...
        // reset classes
        if (modalImage) {
            modalImage.className = "img-fluid rounded shadow-lg loading";
    ...
    async function init() {
        ...
        if (modalCopyUrl) modalCopyUrl.addEventListener("click", copyImageUrl);
        ...
        document.addEventListener("keydown", (e) => {
            // Focus search with / key (when not in input)
        ...
        try {
            ...
            if (
                catObj &&
                Array.isArray(catObj.mobile) &&
                catObj.mobile.length
            ) {
                categories = catObj;
            } else {
                const map = new Map();
            ...
            if (downloadAllBtn) {
                const original =
                    downloadAllBtn.href || "wallpaper-mobile-all.zip";
                ...
                (async () => {
                    try {
                        ...
                        if (url) {
                            ...
                            downloadAllBtn.href =
                                downloadAllBtn.dataset.fallback ||
                                "wallpaper-mobile-all.zip";
                            downloadAllBtn.setAttribute(
                                "download",
                                "wallpaper-mobile-all.zip"
...
(function typingAnimation() {
    ...
    // If mobile (<= 768px) just show the first phrase as static text
